...
@backoff.on_exception(
    ...
    on_backoff=lambda x: resources.LOGGER.exception(
        'Rate Limit Exception replying to comment {}, sleeping. Details: {}'
        .format(x['args'][0].id, str(x))
    )
)
...
def process_comment(comment):
    ...
    if jupy_links:
        logger.info(
            'Found Jupyter link(s) in comment {}'.format(comment_id)
        )
...
def main(subreddits):
    ...
    for comment in comments:
        try:
            ...
            logger.exception('Uncaught exception on comment {}, skipping.'
                             .format(comment.id) + ' Details:')
...
def show_subreddits():
    ...
    msg_relevant = pformat(list(sorted(resources.SUBREDDITS_RELEVANT)),
                           compact=True)
